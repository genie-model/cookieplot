...
%
%   fun_near
%   searches for a grid '1' adjacent to the cell containing a 
%   given lon,lat location and returns found locations
%   (plus orders them by distance of original lon,lat to grid cell center)
%   if no value neighbouring cells are found, all the distances 
%   to each neighbouring cell is set to NaN
%   => if shortest distance is NaN, there are no neighbouring valid cells
%
%   ***********************************************************************
...
for n = 1:n_srch
    ...
    % use simply measure of total degree 'distance'
    loc_d = (loc_di^2 + loc_dj^2)^0.5;
    if (loc_d == 0.0), loc_d =  NaN; end
...
% *********************************************************************** %
%
% return (i,j) location of (lon,lat) input
sout.i  = i;
sout.j  = j;
sout.ij = [i j];
sout.ji = [j i];
